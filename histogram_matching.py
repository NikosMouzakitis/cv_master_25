...
def select_optimal_reference(data_df, n_candidates=300):
    """
    Selects the single best reference image by:
    1. Randomly sampling candidate images
    2. Computing their class-separability score when used as reference
    3. Selecting the candidate that maximizes between-class separation
    """
    print("Finding optimal reference image...")
    
    # Sample candidate references (from either class)
    candidates = data_df.sample(min(n_candidates, len(data_df)))['filepath'].tolist()
    ...
    for ref_path in candidates:
        # Compute class intensity distributions after matching
        class0_means = []
        class1_means = []
        
        # Test on a subset of images for efficiency
        test_images = data_df.groupby('label').apply(lambda x: x.sample(20)).reset_index(drop=True)
        
        for _, row in test_images.iterrows():
            img = sitk.GetArrayFromImage(sitk.ReadImage(row['filepath'], sitk.sitkFloat32))
            if img.ndim == 3:
                img = np.mean(img, axis=0)
            
            # Apply histogram matching
            ref_img = sitk.GetArrayFromImage(sitk.ReadImage(ref_path, sitk.sitkFloat32))
            if ref_img.ndim == 3:
                ref_img = np.mean(ref_img, axis=0)
            matched = histogram_match_images(img, ref_img)
            
            # Store mean intensity by class
            if row['label'] == 0:
                class0_means.append(np.mean(matched))
            else:
                class1_means.append(np.mean(matched))
        
        # Calculate separability score (higher is better)
        score = np.abs(np.mean(class1_means) - np.mean(class0_means)) / (np.std(class0_means) + np.std(class1_means))
        
        if score > best_score:
            best_score = score
            best_ref_path = ref_path
            print(f"New best reference: {best_ref_path} (score: {score:.3f})")
    
    print(f"\nSelected optimal reference: {best_ref_path}")
    print(f"Class separability score: {best_score:.3f}")
...
def main():
    ...
    # Replace your histogram matching section with:
    print("\n=== Unbiased Histogram Matching ===")

    # Step 1: Select optimal single reference
    optimal_ref_path = select_optimal_reference(data_df)
    ref_img = sitk.GetArrayFromImage(sitk.ReadImage(optimal_ref_path, sitk.sitkFloat32))
    if ref_img.ndim == 3:
        ref_img = np.mean(ref_img, axis=0)

    # Step 2: Process all images with this single reference
    features_hm = []
    for idx, row in data_df.iterrows():
        src_img = sitk.GetArrayFromImage(sitk.ReadImage(row['filepath'], sitk.sitkFloat32))
        if src_img.ndim == 3:
            src_img = np.mean(src_img, axis=0)
        
        matched_img = histogram_match_images(src_img, ref_img)
        
        # Convert to SimpleITK image for feature extraction
        matched_sitk = sitk.GetImageFromArray(matched_img)
        
        # Create mask (same as original process_image function)
        mask_array = np.ones_like(matched_img, dtype=np.uint8)
        border_size = 10 
        mask_array[:border_size, :] = 0
        mask_array[-border_size:, :] = 0
    ...
    features_hm_df = pd.DataFrame(features_hm)
    ...
    nn_hm = MLPClassifier(hidden_layer_sizes=(100, 50), 
                         max_iter=500, 
                         random_state=42)
